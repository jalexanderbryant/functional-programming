...
diff :: (Ord a) => Set a -> Set a -> Set a
diff (Set xs)(Set ys) = makeSet (diff_utility xs ys)

diff_utility :: Ord a => [a]->[a]->[a]
diff_utility = foldl (flip delete)


symDiff :: (Ord a) => Set a -> Set a -> Set a
symDiff (Set xs) (Set ys) = diff (union (Set xs) (Set ys))  (inter (Set xs) (Set ys))

buildList :: Set a -> [a]
buildList (Set s) = foldr (:) [] (s)   

powerSet :: (Ord a) => Set a -> Set (Set a)
powerSet s = makeSet $ map (makeSet) (power_set_utility $ buildList s)

power_set_utility :: [a] -> [[a]]
power_set_utility [] = [[]]
power_set_utility (x:xs) = power_set_utility xs ++ map(x:) (power_set_utility xs)
